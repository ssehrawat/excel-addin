...
### Chart Creation vs Modification
New charts must always be created via `chart_inserts` (Option A response), never via `execute_xl_office_js`. Only use `execute_xl_office_js` for modifying existing charts (titles, legend, formatting).

### Chart Sorting
When a user asks to "sort" a chart (e.g. "sort bars highest to lowest"), the system prompt instructs the LLM not to sort the actual sheet data. The user wants the chart visual order changed, not the spreadsheet reordered. The sort example in the `execute_xl_office_js` tool description is only for explicit sheet-data sorting requests.
